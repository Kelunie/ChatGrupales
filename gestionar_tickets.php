...
<body>
    ...
    <!--<audio id="sonidoNuevoTicket" src="path/to/sound.mp3" preload="auto"></audio> -->
    <script>
    $(document).ready(function() {
        const table = $('#tablaTickets').DataTable(); // Inicializa DataTables

        // Aquí puedes agregar un event listener para el filtrado
        table.on('search.dt', function() {
            const data = table.rows().data(); // Obtiene todos los datos de la tabla
            data.each(function(value, index) {
                // Aquí puedes revisar el valor y tomar acciones si es necesario
                if (value[5] === '') { // Si el campo 'tecnico_asignado' es vacío
                    // Manten el valor como 'Sin asignar'
                    value[5] =
                        'Sin asignar'; // Index 5 corresponde a la columna 'Técnico Asignado'
                }
            });
            table.rows().invalidate(); // Actualiza la tabla
        });
    });
    </script>
    <script>
        ...
        fetch('obtener_tickets.php')
            ...
            .then(data => {
                ...
                data.forEach(ticket => {
                    ...
                    <td>${ticket.estado}</td>
                    <td>${ticket.fecha}</td>
                    <td>${ticket.username || 'Sin asignar'}</td> <!-- Mantener el nombre correcto -->
                    <td>
